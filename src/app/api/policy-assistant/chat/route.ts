    ...
    return NextResponse.json(
      {
        ...
        retrieval: {
          ...
          subIssues: retrievalContext.subQueries.map((subQuery) => subQuery.query),
          semanticComparison: {
            policyCandidateCount: retrieval.semanticCandidates.length,
            handbookCandidateCount: handbookRetrieval.semanticCandidates.length,
            policyCandidates: retrieval.semanticCandidates.map((candidate) => ({
              id: candidate.id,
              policySection: candidate.policySection,
              policyCode: candidate.policyCode,
              policyTitle: candidate.policyTitle,
              semanticScore: roundDebugScore(candidate.semanticScore),
              subIssues: candidate.subIssues,
              selectedByCurrentRetrieval: selectedPolicyIds.has(candidate.id),
              excerpt: buildExcerpt(candidate.policyWording, 180),
            })),
            handbookCandidates: handbookRetrieval.semanticCandidates.map((candidate) => ({
              id: candidate.id,
              handbookType: candidate.handbookType,
              sectionTitle: candidate.sectionTitle,
              semanticScore: roundDebugScore(candidate.semanticScore),
              subIssues: candidate.subIssues,
              selectedByCurrentRetrieval: selectedHandbookIds.has(candidate.id),
              excerpt: buildExcerpt(candidate.content, 180),
            })),
          },
